...
<script>
    $(document).ready(function () {
        ...
        $("#table_list").bootstrapTable({
            ...
            columns: [{
                ...
                formatter: function (value, row, index) {
                    if (value == '0')
                        return '<span class="label label-info">未注销</span>';
                ...
                formatter: function (value, row, index) {
                    ...
                    operateHtml = operateHtml + '<@shiro.hasPermission name="system:user:grant"><button class="btn btn-info btn-xs" type="button" onclick="grant(\''+row.id+'\')"><i class="fa fa-arrows"></i>&nbsp;关联 </button>&nbsp;</@shiro.hasPermission>';
                    operateHtml = operateHtml + '<@shiro.hasPermission name="system:user:resume">&nbsp;<button class="btn btn-success btn-xs" type="button" onclick="resume(\''+row.id+'\')"><i class="fa fa-refresh"></i>&nbsp;恢复</button></@shiro.hasPermission>';
                    return operateHtml;
                }
            }]
        });
    ...
    function del(id){
        layer.confirm('确定删除吗?【二次删除将真正删除】', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "${ctx!}/admin/user/delete/" + id +'?uCode='+'<@shiro.principal type="cn.edu.jxnu.base.entity.User" property="userCode"/>',
    ...
    function resume(id){
        layer.confirm('确定恢复该账号吗?', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                type: "POST",
                dataType: "json",
                url: "${ctx!}/admin/user/resume/"+ id,
                success: function(msg){
                    layer.msg(msg.message, {time: 2000},function(){
                        $('#table_list').bootstrapTable("refresh");
                        layer.close(index);
    ...
    $("#btnExport").click(function(){
        ...
        escape:'false',
        fileName: '用户信息',
        ignoreColumn: ['deleteStatus','locked','empty'],//不导入列
        });
